ig.module(
    'game.entities.leak'
  )
  .requires(
    'impact.entity'
  )
  .defines(function () {

    EntityLeak = ig.Entity.extend({

      animSheet: new ig.AnimationSheet('media/spritemap.png', 20, 20),

      size: {x: 20, y: 30},

      leaks: [],

      maxLeaks: 2,

      bounceVel: 200,
      speedThreshold: 300,

      _wmDrawBox: true,
      _wmScalable: true,
      _wmBoxColor: '#bada55',

      type: ig.Entity.TYPE.B,
      checkAgainst: ig.Entity.TYPE.A,
      collides: ig.Entity.COLLIDES.FIXED,

      gravityFactor: 0,

      init: function (x, y, settings) {
        this.parent(x, y, settings);

        this.leaks.push({
          direction: 1,
          position: this.pos.x + this.size.x / 2,
          timer: new ig.Timer(),
          interval: 0.1
        });


        this.leaks.push({
          direction: -1,
          position: this.pos.x + 10,
          timer: new ig.Timer(),
          interval: 0.05
        });

        ig.game.hero = this;
      },

      update: function () {
        var leak;
        var y;
        var x;

        for (var i = 0; i < this.leaks.length; i++) {
          leak = this.leaks[i];

          if (leak.timer.delta() > leak.interval) {

            x = leak.position;
            y = leak.direction === -1 ? this.pos.y : this.pos.y + this.size.y;

            for (var j = 0;  leak.timer.delta() - j > leak.interval; j += leak.interval) {

              ig.game.spawnEntity(EntityLeakParticle, x, y, {
                vel: {x: Math.random() * 30 - 15, y: 500 * leak.direction}
              })

            }

;

            leak.timer.reset();
          }

        }

        this.parent();

      },

      fixLeak: function (start, end, direction) {
        var leak;

        console.log('fixleak', start, end, direction)

        for (var i = 0; i < this.leaks.length; i++) {
          leak = this.leaks[i];

          if (leak.direction === direction && leak.position > start && leak.position < end) {
            this.leaks.splice(i, 1);
          }
        }


      },

      collideWith: function (other, axis) {

        if (other.isHero) {

          if (axis === 'y' && Math.abs(other.prevVel.y) >= this.speedThreshold) {
            other.vel.y = other.prevVel.y < 0 ? 200 : -200;

            this.fixLeak(other.pos.x, other.pos.x + other.size.x, other.vel.y < 0 ? -1 : 1);
          }

        }

        this.parent(other, axis)

      }
    });

    var EntityLeakParticle = EntityParticle.extend(({
      lifetime: 10,
      fadetime: 1,
      bounciness: 0.1,
      vel: {x: 60, y: 20},
      size: {x: 2, y: 2},

      animSheet: new ig.AnimationSheet('media/spritemap.png', 4, 4),

      init: function (x, y, settings) {

        this.addAnim('idle', 5, [10]);
        this.parent(x, y, settings);
      }
    }));

  })